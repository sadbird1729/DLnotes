...
梯度下降法为${\theta_{j}}:={\theta_{j}}-\alpha \frac{\partial }{\partial {\theta_{j}}}J\left( \theta  \right)$

$={\theta_{j}}-a\frac{1}{m}\sum\limits_{i=1}^{m}{\left( \left({{h}_{\theta }}({{x}_{j}^{(i)}})-{{y}^{(i)}} \right)\cdot {{x}_{j}^{(i)}} \right)}$
